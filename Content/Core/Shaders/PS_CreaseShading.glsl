...
void main()
{
    ...
    vec3 fragmentPosition = texture2D(sPosition, vUv0).xyz;
    vec3 fragmentNormal = texture2D(sNormal, vUv0).xyz;
    ...
    for (int i = 0; i < (stippleSize + 1) / 2; i++)
    {
        ...
        for(int j = 0; j < (stippleSize + 1) / 2; j++)
        {
            ...
            vec2 sampleUV = vUv0 + (sampleOffset * cViewportSize.zw * cKernelSize);
